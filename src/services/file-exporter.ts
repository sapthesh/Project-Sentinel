...
import {
  SystemMetrics,
  ExportOptions,
  DEFAULT_SECURITY_CONSTRAINTS as SECURITY_CONSTRAINTS
...
export class SentinelExportError extends Error {
  constructor(
    message: string,
    public code: string = ERROR_CODES.EXPORT_ERROR
  ) {
    super(message);
...
export class FileExporter {
  ...
  async export(
    ...
    const validatedPath = await this.validateAndNormalizePath(
      options.outputPath,
      options.format
    );
    ...
    if (options.samples && options.samples > SECURITY_CONSTRAINTS.maxSamples) {
      throw new SentinelExportError(
        `Sample count exceeds maximum (${SECURITY_CONSTRAINTS.maxSamples})`,
  ...
  async exportToJsonAtomic(
    ...
    try {
      ...
      if (byteLength > SECURITY_CONSTRAINTS.maxFileSizeBytes) {
        throw new SentinelExportError(
          `Content size (${byteLength} bytes) exceeds maximum (${SECURITY_CONSTRAINTS.maxFileSizeBytes} bytes)`,
      ...
      throw new SentinelExportError(
        'Failed to export metrics: ' +
          this.sanitizeErrorMessage(error instanceof Error ? error.message : String(error)),
  ...
  async exportToCsvAtomic(
    ...
    try {
      ...
      if (byteLength > SECURITY_CONSTRAINTS.maxFileSizeBytes) {
        throw new SentinelExportError(
          `Content size (${byteLength} bytes) exceeds maximum`,
          ERROR_CODES.FILE_TOO_LARGE
        );
      }
   ...
   */
  async validateAndNormalizePath(
    rawPath: string,
    ...
    if (rawPath.length > SECURITY_CONSTRAINTS.maxPathLength) {
      throw new SentinelExportError(
        `Path length exceeds maximum (${SECURITY_CONSTRAINTS.maxPathLength} characters)`,
        ERROR_CODES.INVALID_PATH
      );
    }
    ...
    if (rawPath.includes('\0')) {
      throw new SentinelExportError(
        'Invalid path: null bytes not allowed',
    ...
    for (const char of SECURITY_CONSTRAINTS.forbiddenPathChars) {
      if (rawPath.includes(char)) {
        throw new SentinelExportError(
          'Invalid path: forbidden characters detected',
          ERROR_CODES.PATH_TRAVERSAL
        );
      }
    }
    ...
    const reservedNames = PATH_SECURITY.reservedNames as readonly string[];
    for (const part of pathParts) {
      const upperPart = part.toUpperCase();
      if (reservedNames.includes(upperPart)) {
        throw new SentinelExportError(
          `Invalid path: reserved name "${part}" not allowed`,
          ERROR_CODES.INVALID_PATH
        );
      }
    }
    ...
    const extension = FILE_EXTENSIONS[expectedFormat as keyof typeof FILE_EXTENSIONS] ||
      `.${expectedFormat}`;
    ...
    throw new SentinelExportError(
      'Invalid path: cannot write to specified location',
