...
export class MetricsCollector {
  private cachedHostInfo: HostInfo | null = null;
  private cpuInitialized = false;
  ...
  private async collectCpuMetrics(): Promise<CpuMetrics> {
    try {
      ...
      const safeFrequency = this.boundNumber(
        cpuInfo.speed || 0,
        0,
        10000
      );
      const safeMaxFrequency = this.boundNumber(
        cpuInfo.speedMax || safeFrequency,
        0,
        10000
      );
      ...
      return {
        ...
        activeCores: Math.min(
          Math.floor((safeUsage / 100) * (cpuInfo.cores || 1)),
          cpuInfo.cores || 1
        ),
        frequency: safeFrequency,
        maxFrequency: safeMaxFrequency,
        temperature: (cpuInfo as any).temperature
          ? this.boundNumber((cpuInfo as any).temperature, -50, 200)
          : undefined,
  ...
  private async collectMemoryMetrics(): Promise<MemoryMetrics> {
    try {
      ...
      const percentage = this.boundPercentage(
  ...
  private async collectHostInfo(): Promise<HostInfo> {
    try {
      ...
      return {
        uptime: Math.max(0, (osInfo as any).uptime || 0),
        hostname: this.sanitizeHostname(osInfo.hostname || 'unknown'),
        platform: this.validatePlatform(osInfo.platform),
        release: (osInfo as any).osrelease || (osInfo as any).distro?.release || 'unknown',
  ...
  async collectSamples(
    ...
    if (count > DEFAULT_SECURITY_CONSTRAINTS.maxSamples) {
      throw new Error(
        `Sample count exceeds maximum allowed (${DEFAULT_SECURITY_CONSTRAINTS.maxSamples})`
      );
    }
    ...
    try {
      for (let i = 0; i < count; i++) {
        ...
        if (heapUsed > DEFAULT_SECURITY_CONSTRAINTS.maxMemoryBytes) {
          throw new Error(
            'Memory limit reached. Aborting sample collection.'
          );
        }
  ...
  private validatePlatform(
    ...
    if (
      ...
      validPlatforms.includes(platform as 'win32' | 'darwin' | 'linux')
    ) {
   ...
   */
  private async collectDiskMetrics(): Promise<import('../models/metrics.js').DiskMetrics> {
    try {
   ...
   */
  private async collectNetworkMetrics(): Promise<import('../models/metrics.js').NetworkMetrics> {
    try {
      const netStats = await si.networkStats();
      const firstInterface = netStats.find(i => (i as any).mac && (i as any).mac !== '00:00:00:00:00:00') || netStats[0];
      ...
      return {
        receivedBytes: Math.max(0, (firstInterface as any).rx_bytes || 0),
        transmittedBytes: Math.max(0, (firstInterface as any).tx_bytes || 0)
      };
    } catch (error) {
