...
export class ReportGenerator {
  ...
  generateSnapshotReport(
    ...
    // CPU section
    lines.push(
      `${chalk.blue('CPU:')}\t\t${metrics.cpu.usage.toFixed(1)}% ` +
        `(Active: ${metrics.cpu.activeCores}/${metrics.cpu.cores} cores)`
    );

    if (verbose) {
      lines.push(
        `  Frequency: ${this.formatFrequency(metrics.cpu.frequency)} MHz`
      );
    ...
    // Memory section
    lines.push(
      `${chalk.blue('Memory:')}\t${this.formatBytes(metrics.memory.used)} / ` +
        `${this.formatBytes(metrics.memory.total)} (${metrics.memory.percentage.toFixed(1)}%)`
    );

    if (verbose) {
      lines.push(
        `  Swap: ${this.formatBytes(metrics.memory.swapUsed)} / ` +
    ...
    // Uptime section
    lines.push(
      `${chalk.blue('Uptime:')}\t${this.formatDuration(metrics.host.uptime)}`
    );
    ...
    if (verbose) {
      ...
      lines.push(
        `${chalk.blue('OS:')}\t${metrics.host.platform} ${metrics.host.release}`
      );
   ...
   */
  private ensureSerializable(metrics: SystemMetrics): SystemMetrics {
    return {
      ...metrics,
      cpu: {
        ...metrics.cpu,
        ...
        temperature:
          metrics.cpu.temperature !== undefined
            ...
            : undefined,
        loadAverage1:
          metrics.cpu.loadAverage1 !== undefined
            ? Number(metrics.cpu.loadAverage1)
            : undefined,
        loadAverage5:
          metrics.cpu.loadAverage5 !== undefined
            ? Number(metrics.cpu.loadAverage5)
            : undefined,
        loadAverage15:
          metrics.cpu.loadAverage15 !== undefined
   ...
   */
  private sanitizeForHtml(history: MetricsHistory): Record<string, unknown> {
   ...
   */
  private buildCpuRow(usage: number, threshold: number): string {
   ...
   */
  private buildMemoryRow(usage: number, threshold: number): string {
    const status = this.getStatusIcon(usage, threshold);
    const color = this.getColorForPercentage(usage, threshold);
    const bar = this.buildProgressBar(usage);
   ...
   * Build disk row
   */
  private buildDiskRow(disk?: any): string {
    ...
    return (
      BOX_CHARS.vertical +
      '  Disk:       ' +
      (chalk as any)[pct > 80 ? 'red' : 'green'](bar) +
   ...
   * Build network row
   */
  private buildNetworkRow(network?: any): string {
   ...
   */
  private getStatusIcon(usage: number, threshold: number): string {
   ...
   */
  private getColorForPercentage(usage: number, threshold: number): string {
    if (usage >= threshold + 10) {
      return 'red';
    } else if (usage >= threshold) {
      return 'yellow';
  ...
  private formatBytes(bytes: number): string {
    ...
    return (
      (bytes / Math.pow(1024, unitIndex)).toFixed(1) +
      ' ' +
      units[unitIndex]
    );
  }
