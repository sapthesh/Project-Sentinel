 ...
 */
export async function handleSnapshot(options: SnapshotOptions): Promise<void> {
 ...
 */
function validateFormat(format: string): 'text' | 'json' {
  const lowerFormat = format.toLowerCase();

  if (VALID_SNAPSHOT_FORMATS.includes(lowerFormat as 'text' | 'json')) {
    return lowerFormat as 'text' | 'json';
  }

  logger.warn(
    `Invalid format "${format}", using default 'text' format`
  );
  return 'text';
}
