...
import ora from 'ora';
import { ExportOptions, ExportFormat, SystemMetrics } from '../models/metrics.js';
...
export async function handleExport(
  ...
  try {
    ...
    let metricsData: SystemMetrics | SystemMetrics[];
    ...
    await exporter.export(metricsData, {
      outputPath,
      format: validatedOptions.format as ExportOptions['format'],
...
function validateExportOptions(options: {
  ...
  const result: ExportOptions = {
    ...
    format: (options.format || DEFAULT_EXPORT_FORMAT) as ExportFormat,
  ...
  const validFormats: readonly ExportFormat[] = ['json', 'csv', 'html'];
  if (!validFormats.includes(result.format as ExportFormat)) {
    logger.warn(
      `Invalid format "${options.format}", using default '${DEFAULT_EXPORT_FORMAT}'`
    );
  ...
  if (samplesVal <= 0) {
    ...
    logger.warn(
      `Sample count ${samplesVal} exceeds maximum (${SECURITY_CONSTRAINTS.maxSamples}), adjusting`
    );
  ...
  if (intervalVal <= 0 || intervalVal > 300000) {
    logger.warn(
      `Interval ${intervalVal}ms out of range, using default ${DEFAULT_EXPORT_INTERVAL}ms`
    );
