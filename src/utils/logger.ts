...
export class Logger {
  ...
  constructor(
    debug: boolean = false,
    logLevel: number = DEFAULT_LOG_LEVEL
  ) {
  ...
  success(message: string): void {
    if (this.logLevel > LOG_LEVELS.INFO) return;
    console.log(
      chalk[COLORS.success](`${STATUS_ICONS.success} ${message}`)
    );
  }
  ...
  warn(message: string): void {
    ...
    console.log(
      chalk[COLORS.warning](`${STATUS_ICONS.warning} ${message}`)
    );
  }
  ...
  error(message: string): void {
    ...
    console.error(
      chalk[COLORS.error](`${STATUS_ICONS.error} ${sanitizedMessage}`)
    );
  }
  ...
  header(title: string): void {
    ...
    console.log(
      chalk[COLORS.primary](
        `\u2502   ${title.padEnd(borderLength - 4)}   \u2502`
      )
    );
    console.log(chalk[COLORS.primary](`\u255A${border}\u255D`));
    console.log();
  ...
  progress(current: number, total: number, message: string): void {
    ...
    const bar =
      chalk[COLORS.success](
        '\u2588'.repeat(filled)
      ) + chalk[COLORS.dim]('\u2591'.repeat(empty));

    console.log(
      chalk.dim(
        `[${bar}] ${percentage}% - ${message} (${current}/${total})`
      )
    );
  }
