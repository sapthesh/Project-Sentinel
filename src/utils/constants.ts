...
import { DEFAULT_SECURITY_CONSTRAINTS, SecurityConstraints } from '../models/metrics.js';
...
export const OPTION_RANGES = {
  monitorInterval: { min: 100, max: 60000, unit: 'ms' } as const,
  monitorDuration: { min: 1, max: 3600, unit: 'seconds' } as const,
  cpuThreshold: { min: 0, max: 100, unit: 'percent' } as const,
  memoryThreshold: { min: 0, max: 100, unit: 'percent' } as const,
  exportSamples: { min: 1, max: 10000, unit: 'samples' } as const,
  exportInterval: { min: 100, max: 300000, unit: 'ms' } as const
};
 ...
 */
export const VALID_EXPORT_FORMATS = ['json', 'csv', 'html'] as const;

/**
 * Valid snapshot output formats
 */
export const VALID_SNAPSHOT_FORMATS = ['text', 'json'] as const;

/**
 * Platform detection
 */
export const PLATFORM = process.platform as 'win32' | 'darwin' | 'linux';
 ...
 */
export const SECURITY_CONSTRAINTS: SecurityConstraints =
...
export const PATH_SECURITY = {
  ...
  traversalPatterns: ['..', '\\\\', '%00', '%2e%2e'] as const,
