...
import { APP_NAME, APP_VERSION, APP_DESCRIPTION, DEFAULT_MONITOR_INTERVAL, DEFAULT_CPU_THRESHOLD, DEFAULT_MEMORY_THRESHOLD, DEFAULT_EXPORT_FORMAT, DEFAULT_EXPORT_SAMPLES, DEFAULT_EXPORT_INTERVAL, SECURITY_CONSTRAINTS } from './utils/constants.js';
/**
...
export function setupCLI() {
    ...
    program
        ...
        .option('-i, --interval <ms>', 'Refresh interval in milliseconds', validateNumberInRange(DEFAULT_MONITOR_INTERVAL.toString(), 100, 60000))
        .option('-d, --duration <sec>', 'Auto-stop after duration (seconds)', validateNumberInRange(null, 1, 3600))
        .option('-q, --quiet', 'Suppress colored output')
        .option('--cpu <pct>', 'CPU warning threshold (%)', validateNumberInRange(DEFAULT_CPU_THRESHOLD.toString(), 0, 100))
        .option('--mem <pct>', 'Memory warning threshold (%)', validateNumberInRange(DEFAULT_MEMORY_THRESHOLD.toString(), 0, 100))
        .action((opts) => {
    ...
    program
        ...
        .option('-f, --format <fmt>', 'Output format (text/json)', validateFormatOption)
        .action((opts) => {
    ...
    program
        ...
        .option('-f, --format <fmt>', 'Export format (json/csv/html)', validateExportFormat)
        .option('-n, --samples <n>', 'Number of samples to collect', validateNumberInRange(DEFAULT_EXPORT_SAMPLES.toString(), 1, SECURITY_CONSTRAINTS.maxSamples))
        .option('-i, --interval <ms>', 'Interval between samples in milliseconds', validateNumberInRange(DEFAULT_EXPORT_INTERVAL.toString(), 100, 300000))
        ...
        const samples = parseInt(opts.samples, 10) || DEFAULT_EXPORT_SAMPLES;
        if (samples < 1 || samples > SECURITY_CONSTRAINTS.maxSamples) {
            logger.error(`Error: Samples must be between 1 and ${SECURITY_CONSTRAINTS.maxSamples}`);
            process.exit(1);
        }
        const interval = parseInt(opts.interval, 10) || DEFAULT_EXPORT_INTERVAL;
 ...
 */
function validateNumberInRange(defaultValue, min, max) {
    return (value) => {
 ...
 */
function validateFormatOption(value) {
