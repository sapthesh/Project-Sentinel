...
import { DEFAULT_MONITOR_INTERVAL, DEFAULT_MONITOR_DURATION, DEFAULT_CPU_THRESHOLD, DEFAULT_MEMORY_THRESHOLD, STATUS_ICONS, SECURITY_CONSTRAINTS } from '../utils/constants.js';
 ...
 */
export async function handleMonitor(options) {
    ...
    try {
        ...
        await startMonitoringLoop(collector, generator, validatedOptions, cleanup);
    }
    catch (error) {
        spinner.stop();
...
function validateMonitorOptions(options) {
    ...
    if (result.interval < 100 || result.interval > 60000) {
        logger.warn(`Interval ${result.interval}ms out of range, using default ${DEFAULT_MONITOR_INTERVAL}ms`);
    ...
    if (result.duration != null && typeof result.duration === 'number') {
        if (result.duration < 1 || result.duration > 3600) {
            logger.warn(`Duration ${result.duration}s out of range, using default (indefinite)`);
            result.duration = DEFAULT_MONITOR_DURATION;
        }
    }
    // Validate thresholds (0-100%)
    if (result.cpuThreshold < 0 || result.cpuThreshold > 100) {
        logger.warn(`CPU threshold ${result.cpuThreshold}% out of range, using default ${DEFAULT_CPU_THRESHOLD}%`);
        result.cpuThreshold = DEFAULT_CPU_THRESHOLD;
    }
    if (result.memoryThreshold < 0 || result.memoryThreshold > 100) {
        logger.warn(`Memory threshold ${result.memoryThreshold}% out of range, using default ${DEFAULT_MEMORY_THRESHOLD}%`);
 ...
 */
function displayInitialReport(metrics, options) {
    logger.clear();
    ...
    const display = new ReportGenerator().generateMonitorDisplay(metrics, {
        cpuThreshold: options.cpuThreshold,
        memoryThreshold: options.memoryThreshold
    });
    console.log(display);
 ...
 */
async function startMonitoringLoop(collector, generator, options, cleanup) {
    let startTime = null;
    let isRunning = true;
 ...
 */
async function displayFinalSummary(samples, _options) {
    logger.clear();
    if (samples.length === 0) {
 ...
 */
function setupCleanup(_duration) {
